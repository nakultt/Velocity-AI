...
const AppLayout = ({ children }: AppLayoutProps) => {
  ...
  const { setMode, isWorkspace } = useMode();
  const toggleMode = () => setMode(isWorkspace ? "personal" : "workspace");
  ...
  const navItems = useMemo(() => {
    ...
    if (isWorkspace) {
      shared.push({ icon: Briefcase, label: "Company", path: "/company-dashboard" });
    }
    shared.push(
      { icon: Activity, label: "AI Activity", path: "/ai-activity" },
      ...
      >
        ...
        <div className="p-3 border-t border-sidebar-border mt-auto">
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition text-sm font-medium ${
              showSidebar ? "" : "justify-center"
            } bg-sidebar-accent/30 hover:bg-sidebar-accent text-sidebar-foreground`}
            title={showSidebar ? undefined : `Switch to ${isWorkspace ? "Student" : "Workspace"}`}
          >
            {isWorkspace ? <Building2 size={18} /> : <GraduationCap size={18} />}
            {showSidebar && (
              <span className="flex items-center gap-2 flex-1">
                <span>{isWorkspace ? "Workspace" : "Student"}</span>
                <ArrowLeftRight size={14} className="ml-auto opacity-50" />
              </span>
            )}
          </button>
        </div>
